...
const securityConfigSchema = z.object({
  ...
  maxFileSize: z.number().min(1024).max(100 * 1024 * 1024).default(50 * 1024 * 1024),
  allowedFileExtensions: z.array(z.string()).default([
    'pdf', 'doc', 'docx', 'txt', 'png', 'jpg', 'jpeg', 'gif',
    'xls', 'xlsx', 'csv', 'zip', 'json', 'xml'
  ]),
...
export class ConfigurationManager {
  ...
  private loadFromEnvironment(): Partial<SecurityConfig> {
    return {
      ...
      maxBulkSize: process.env.JIRA_MAX_BULK_SIZE ? parseInt(process.env.JIRA_MAX_BULK_SIZE) : undefined,
      maxFileSize: process.env.JIRA_MAX_FILE_SIZE ? parseInt(process.env.JIRA_MAX_FILE_SIZE) : undefined,
      ...
      auditLogging: {
        ...
        retentionDays: process.env.JIRA_AUDIT_RETENTION ? parseInt(process.env.JIRA_AUDIT_RETENTION) : 30,
      },
      permissionChecking: {
        enabled: process.env.JIRA_PERMISSION_CHECK !== 'false',
        strict: process.env.JIRA_PERMISSION_STRICT === 'true',
        cacheTimeout: process.env.JIRA_PERMISSION_CACHE ? parseInt(process.env.JIRA_PERMISSION_CACHE) : 300,
      },
    };
  }
  ...
  needsConfirmation(operation: string): boolean {
    ...
    if (operation.includes('sprint') && (operation.includes('complete') || operation.includes('start'))) {
...
export const isOperationAllowed = (operation: string) => securityConfig.isOperationAllowed(operation);
export const needsConfirmation = (operation: string) => securityConfig.needsConfirmation(operation);
export const validateOperation = (operation: string, context?: any) => securityConfig.validateOperation(operation, context);