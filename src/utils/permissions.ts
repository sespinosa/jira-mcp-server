...
export class PermissionError extends Error {
  constructor(message: string, public permission: string, public resource?: string) {
    super(message);
...
export const operationPermissions: Record<string, OperationPermissions> = {
  ...
  'create_issue': {
    ...
    permissions: [
      { permission: 'CREATE_ISSUES', required: true }
    ],
  ...
  'link_issues': {
    ...
    permissions: [
      { permission: 'LINK_ISSUES', required: true }
    ],
  ...
  'upload_attachment': {
    ...
    permissions: [
      { permission: 'CREATE_ATTACHMENTS', required: true }
    ],
  ...
  'create_sprint': {
    ...
    permissions: [
      { permission: 'MANAGE_SPRINTS', required: true }
    ],
  ...
  'move_issues_to_sprint': {
    ...
    permissions: [
      { permission: 'SCHEDULE_ISSUES', required: true }
    ],
  ...
  'create_project_component': {
    ...
    permissions: [
      { permission: 'ADMINISTER_PROJECTS', required: true }
    ],
  ...
  'create_board': {
    ...
    permissions: [
      { permission: 'MANAGE_BOARDS', required: true }
    ],
...
export class PermissionValidator {
  ...
  async checkOperationPermissions(
    ...
    try {
      ...
      if (missingOptional.length > 0) {
        auditLogger.logOperation('permission_warning', 'operation', {
          operation,
          missingOptional,
          context
        }, { riskLevel: 'low' });
      ...
      auditLogger.logFailure('permission_check', operation, error instanceof Error ? error.message : 'Unknown error', {
        operation,
        context
      });
  ...
  private hasPermission(userPermissions: string[], requiredPermission: string): boolean {
    ...
    const permissionHierarchy: Record<string, string[]> = {
      ...
      'ADMINISTER_PROJECTS': [
        'CREATE_ISSUES', 'EDIT_ISSUES', 'DELETE_ISSUES',
        'MANAGE_SPRINTS', 'SCHEDULE_ISSUES',
