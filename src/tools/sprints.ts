  ...
  server.registerTool(
    ...
    async (args) => {
      try {
        ...
        await withRateLimit(
          async () => {
            auditLogger.logOperation('get_board_sprints', 'sprint', {
  ...
  server.registerTool(
    ...
    async (args) => {
      try {
        ...
        await withRateLimit(
          async () => {
            auditLogger.logOperation('get_sprint_issues', 'sprint', {
              sprintId: args.sprintId,
              jql: args.jql,
            });
            return true;
        ...
        auditLogger.logFailure(
          ...
          'sprint',
          error instanceof Error ? error.message : 'Unknown error',
          { sprintId: args.sprintId, jql: args.jql }
        );
  ...
  server.registerTool(
    ...
    async (args) => {
      try {
        ...
        await withRateLimit(
          async () => {
            auditLogger.logOperation(
              'create_sprint',
              'sprint',
              { boardId: args.boardId, name: args.name },
              {
                riskLevel: 'high',
              }
            );
        ...
        auditLogger.logSuccess('create_sprint', 'sprint', {
          sprintId: sprint.id,
          boardId: validatedArgs.boardId,
          name: validatedArgs.name,
        });
